...
private:
	// <item_id, stock_ptr>
	std::map<unsigned int, std::shared_ptr<Stock> > _stocks; // shared_ptr을 typedef 로 빼면 꺽쇄 붙여도 됨.
...
public:
	...
	const std::shared_ptr<Stock> findStockByItemId(const unsigned int itemId) const;
